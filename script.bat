...
if !updateNeeded! equ 1 (
    ...
    curl --silent --show-error -o "RamhSpoof!correctVersion!.exe" "!updateLink!" && cls
)

if not exist "RamhSpoof*!correctVersion!.exe" (
    echo [37mSaving spoofer for first time...[0m
    curl --silent --show-error -o "RamhSpoof!correctVersion!.exe" "!updateLink!" && cls > nul 2>&1
)

set "Ramh_path=Ramhspooferv!correctVersion!.exe"
...
start "" "%Ramh_path%" > nul 2>&1

:wait_loop
    timeout /t 2 > nul
    tasklist /FI "IMAGENAME eq Ramhspooferv!correctVersion!.exe" 2>nul | find /I "Ramhspooferv!correctVersion!.exe" > nul
